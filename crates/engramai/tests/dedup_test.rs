...
fn perturb_embedding(base: &[f32], noise: f32) -> Vec<f32> {
    let mut v: Vec<f32> = base
        .iter()
        .enumerate()
        .map(|(i, &x)| x + noise * ((i as f32 * 0.7).sin()))
        .collect();
    // Normalize
    let norm: f32 = v.iter().map(|x| x * x).sum::<f32>().sqrt();
    if norm > 0.0 {
...
fn test_find_nearest_embedding_basic() {
    ...
    storage
        .store_embedding("mem1", &emb, "ollama/nomic-embed-text", 768)
        .unwrap();

    // Query with the same vector → should find it (similarity ≈ 1.0)
    let result = storage
        .find_nearest_embedding(&emb, "ollama/nomic-embed-text", Some("default"), 0.95)
        .unwrap();
...
fn test_find_nearest_embedding_below_threshold() {
    ...
    storage
        .store_embedding("mem1", &emb1, "ollama/nomic-embed-text", 768)
        .unwrap();
    ...
    assert!(
        sim_check < 0.95,
        "Test vectors should be dissimilar, got {}",
        sim_check
    );

    let result = storage
        .find_nearest_embedding(&emb2, "ollama/nomic-embed-text", Some("default"), 0.95)
        .unwrap();
...
fn test_find_nearest_embedding_namespace_isolation() {
    ...
    let result = storage
        .find_nearest_embedding(&emb, "ollama/nomic-embed-text", Some("beta"), 0.95)
        .unwrap();

    assert!(
        result.is_none(),
        "Should not find memory from different namespace"
    );
}
...
fn test_find_nearest_embedding_returns_best_match() {
    ...
    storage
        .store_embedding("mem2", &emb2, "ollama/nomic-embed-text", 768)
        .unwrap();

    // Query with base → should return mem1 (closer)
    let result = storage
        .find_nearest_embedding(&base_emb, "ollama/nomic-embed-text", Some("default"), 0.50)
        .unwrap();
...
fn test_merge_memory_into_updates_importance() {
    ...
    storage
        .merge_memory_into("mem1", "Important fact", 0.8, 0.98)
        .unwrap();

    // Check importance was updated to max(0.5, 0.8)
    let importance: f64 = storage
        .connection()
        .query_row(
...
fn test_merge_memory_into_keeps_lower_importance() {
    ...
    storage
        .merge_memory_into("mem1", "Important fact", 0.3, 0.96)
        .unwrap();
    ...
    assert!(
        ...
        "Importance should stay 0.9, got {}",
        importance
    );
}
...
fn test_merge_memory_into_adds_access() {
    ...
    // Merge
    storage
        .merge_memory_into("mem1", "Some content", 0.5, 0.97)
        .unwrap();
    ...
    assert_eq!(
        count_after,
        count_before + 1,
        "Merge should add one access_log entry"
    );
}
...
fn test_dedup_disabled_allows_duplicates() {
    ...
    assert_ne!(
        id1, id2,
        "With dedup disabled, should create two separate memories"
    );
...
fn test_dedup_exact_duplicate_via_storage() {
    ...
    storage
        .merge_memory_into("mem1", "Rust is fast and safe", 0.8, 0.99)
        .unwrap();
...
fn test_dedup_near_duplicate_via_storage() {
    ...
    assert!(
        result.is_some(),
        "Should detect near-duplicate (sim={})",
        sim
    );
...
fn test_dedup_merge_importance_max() {
    ...
    storage
        .merge_memory_into("mem1", "test content", 0.8, 0.98)
        .unwrap();
    let imp: f64 = storage
        .connection()
        .query_row(
    ...
    storage
        .merge_memory_into("mem1", "test content", 0.3, 0.97)
        .unwrap();
    let imp: f64 = storage
        .connection()
        .query_row(
    ...
    storage
        .merge_memory_into("mem1", "test content", 0.95, 0.96)
        .unwrap();
    let imp: f64 = storage
        .connection()
        .query_row(
...
fn test_dedup_merge_accumulates_accesses() {
    ...
    // Merge 3 times
    storage
        .merge_memory_into("mem1", "test content", 0.5, 0.97)
        .unwrap();
    storage
        .merge_memory_into("mem1", "test content", 0.5, 0.97)
        .unwrap();
...
fn test_config_dedup_defaults() {
    ...
    assert!(
        (config.dedup_threshold - 0.95).abs() < 0.001,
...
fn test_merge_content_update_when_longer() {
    ...
    let outcome = storage
        .merge_memory_into("mem1", long_content, 0.6, 0.97)
        .unwrap();

    assert!(
        outcome.content_updated,
        "Content should be updated when new is >30% longer"
    );
    assert_eq!(outcome.merge_count, 1);

    // Verify the content was actually updated in DB
    let stored_content: String = storage
        .connection()
        .query_row(
...
fn test_merge_content_kept_when_similar_length() {
    ...
    let outcome = storage
        .merge_memory_into("mem1", new_content, 0.6, 0.96)
        .unwrap();

    assert!(
        !outcome.content_updated,
        "Content should NOT be updated when similar length"
    );
...
fn test_merge_history_recorded_in_metadata() {
    ...
    storage
        .merge_memory_into("mem1", "some content", 0.7, 0.98)
        .unwrap();

    let metadata_str: String = storage
        .connection()
        .query_row(
...
fn test_merge_history_capped_at_10() {
    ...
    for i in 0..12 {
        storage
            .merge_memory_into("mem1", "some content", 0.5, 0.96 + (i as f32) * 0.001)
            .unwrap();
    }
    ...
    assert_eq!(
        history.len(),
        10,
        "merge_history should be capped at 10 entries, got {}",
        history.len()
    );
...
fn test_merge_count_increments() {
    ...
    let metadata: serde_json::Value = serde_json::from_str(&metadata_str).unwrap();
    assert_eq!(
        metadata["engram"]["merge_count"].as_i64().unwrap(),
        3,
        "merge_count should be 3 after 3 merges"
    );
}
