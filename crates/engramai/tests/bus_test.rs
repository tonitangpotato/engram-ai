...
use engramai::bus::accumulator::{
    EmpathyAccumulator, MIN_EVENTS_FOR_SUGGESTION, NEGATIVE_THRESHOLD,
};
use engramai::bus::alignment::{calculate_importance_boost, score_alignment, ALIGNMENT_BOOST};
use engramai::bus::feedback::{
    BehaviorFeedback, LOW_SCORE_THRESHOLD, MIN_ATTEMPTS_FOR_SUGGESTION as MIN_BEHAVIOR_ATTEMPTS,
};
use engramai::bus::mod_io::{parse_heartbeat, parse_identity, parse_soul, Drive};
use engramai::{EmpathyBus, Memory, MemoryConfig, MemoryType};
...
fn test_emotional_accumulator() {
    ...
    assert!(
        (coding_trend.valence - 0.6).abs() < 0.01,
        "Expected ~0.6, got {}",
        coding_trend.valence
    );

    let debug_trend = acc.get_trend("debugging").unwrap().unwrap();
    assert_eq!(debug_trend.count, 2);
    assert!(
        (debug_trend.valence - (-0.6)).abs() < 0.01,
        "Expected ~-0.6, got {}",
        debug_trend.valence
    );
...
fn test_emotional_accumulator_threshold() {
    ...
    assert!(needing_update
        .iter()
        .any(|t| t.domain == "frustrating_task"));
}
...
fn test_drive_alignment_boosts_memory_importance() {
    ...
    mem.add(aligned_content, MemoryType::Episodic, None, None, None)
        .unwrap();

    // Store unaligned memory
    let unaligned_content = "xyz abc 123 random text";
    mem.add(unaligned_content, MemoryType::Episodic, None, None, None)
        .unwrap();
    ...
    let aligned_result = results
        .iter()
        .find(|r| r.record.content.contains("learned"));
...
fn test_behavior_feedback_low_score_flagging() {
    ...
    assert!(
        stats.should_deprioritize(),
        "Should suggest deprioritization"
    );
...
fn test_bus_integration() {
    ...
    for _ in 0..10 {
        bus.process_interaction(mem.connection(), "more frustration", -0.6, "debugging")
            .unwrap();
    }
...
fn test_bus_heartbeat_suggestions() {
    ...
    assert!(heartbeat_updates
        .iter()
        .any(|h| h.suggestion == "deprioritize"));
}
...
fn test_full_emotional_loop() {
    ...
    let bus = mem.empathy_bus().unwrap();
    for _ in 0..5 {
        bus.log_behavior(mem.connection(), "helpful_action", true)
            .unwrap();
    }
    for _ in 0..12 {
        bus.log_behavior(mem.connection(), "unhelpful_action", false)
            .unwrap();
    }
    ...
    assert!(heartbeat_suggestions
        .iter()
        .any(|h| h.action == "unhelpful_action"));
