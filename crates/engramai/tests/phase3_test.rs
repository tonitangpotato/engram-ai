...
fn test_cross_namespace_hebbian() {
    ...
    let _ = mem
        .recall_from_namespace("oil energy", 10, None, None, Some("*"))
        .unwrap();
    let _ = mem
        .recall_from_namespace("oil energy crisis", 10, None, None, Some("*"))
        .unwrap();

    // Use recall_with_associations to also trigger cross-namespace hebbian
    let result = mem
        .recall_with_associations("oil energy", Some("*"), 10)
        .unwrap();
    ...
    assert!(
        namespaces.contains(&"engine".to_string()) || namespaces.contains(&"alpha".to_string())
    );

    // Check cross-namespace links were formed
    let cross_assocs = mem
        .get_cross_associations(&engine_id)
        .ok()
        .unwrap_or_default();
...
fn test_subscription_threshold() {
    ...
    assert!(
        notifs.is_empty(),
        "Low importance memory should not trigger notification"
    );
...
fn test_recall_with_associations() {
    ...
    let result = mem
        .recall_with_associations("anomaly", Some("*"), 10)
        .unwrap();
    ...
    // Final recall
    let result = mem
        .recall_with_associations("anomaly system", Some("*"), 10)
        .unwrap();
...
fn test_cross_links_acl() {
    ...
    assert!(
        links.is_empty(),
        "Agent without read permission should not see cross-links"
    );
...
fn test_subscription_wildcard() {
    ...
    assert_eq!(
        notifs.len(),
        3,
        "Should receive notifications from all namespaces"
    );
