...
fn setup_memory_with_association() -> Memory {
    ...
    config.entity_config.known_projects = vec!["rust".to_string(), "python".to_string()];
    config.entity_config.known_people = vec!["alice".to_string(), "bob".to_string()];
...
fn test_write_time_association_creates_links() {
    ...
    assert!(
        count >= 1,
        "should find at least 1 link between memories, found {}",
        count
    );
}
...
fn test_association_disabled_no_links() {
    ...
    assert_eq!(
        count, 0,
        "should have no links when association is disabled"
    );
}
...
fn test_differential_decay_in_consolidation() {
    ...
    let corecall_before =
        get_strength(mem.connection(), &id_a, &id_c).expect("corecall link should exist");
...
fn test_association_failure_does_not_block_memory_storage() {
    ...
    assert!(
        content.contains("stored regardless"),
        "memory content should be persisted"
    );
}
