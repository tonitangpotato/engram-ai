...
use engramai::{
    Memory, MemoryConfig, MemoryType, Predicate, Triple, TripleExtractor, TripleSource,
};
...
fn test_triple_entities_appear_in_get_entities_for_memory() {
    ...
    assert!(
        entities.contains(&"engram".to_string()),
        "missing 'engram' in {:?}",
        entities
    );
    assert!(
        entities.contains(&"act-r".to_string()),
        "missing 'act-r' in {:?}",
        entities
    );
    assert!(
        entities.contains(&"hebbian learning".to_string()),
        "missing 'hebbian learning' in {:?}",
        entities
    );
    assert!(
        entities.contains(&"neural mechanism".to_string()),
        "missing 'neural mechanism' in {:?}",
        entities
    );
}
...
fn test_consolidate_with_triple_enabled_extracts() {
    ...
    let (subj, pred, obj): (String, String, String) = mem
        .connection()
        .query_row(
...
fn test_consolidate_with_failing_extractor_is_nonfatal() {
    ...
    let id = mem
        .add("test content", MemoryType::Factual, Some(0.5), None, None)
        .expect("add");

    // Should not panic or return error
    mem.consolidate(1.0)
        .expect("consolidate should succeed even with failing extractor");
...
fn test_consolidate_with_empty_extractor_increments_attempts() {
    ...
    let id = mem
        .add("ok sounds good", MemoryType::Factual, Some(0.5), None, None)
        .expect("add");
...
fn test_predicate_from_str_lossy_all_variants() {
    ...
    assert_eq!(
        Predicate::from_str_lossy("depends_on"),
        Predicate::DependsOn
    );
    ...
    assert_eq!(
        Predicate::from_str_lossy("implements"),
        Predicate::Implements
    );
    assert_eq!(
        Predicate::from_str_lossy("contradicts"),
        Predicate::Contradicts
    );
    assert_eq!(
        Predicate::from_str_lossy("related_to"),
        Predicate::RelatedTo
    );
}
...
fn test_triple_config_missing_from_json_uses_defaults() {
    ...
    let mut config_value: serde_json::Value =
        serde_json::to_value(MemoryConfig::default()).expect("serialize");
