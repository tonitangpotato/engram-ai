...
// ============================================================================
// ISS-178: slim prev-turn ExtractionContext
// ============================================================================
//
// Why this exists:
// ISS-161 audit showed q3 ("Caroline's adoption agencies") fails because the
// extractor sees only the assistant turn ("Researching adoption agencies")
// without the user turn that names what's being researched. The noun-phrase
// "adoption agencies" survives only if the prev turn is visible.
//
// Why slim (current + Option<prev>) instead of ISS-162's full SessionState:
// Evidence from ISS-161 supports prev-turn for 1-2 questions out of 9
// single-fact misses. The full session-state machinery (rolling summary,
// per-namespace queue, SessionState type) is not justified by today's
// evidence. Keep this surface minimum until A/B confirms lift.
//
// Why a new method (`extract_with_context`) instead of changing `extract`:
// 19+ call sites use `extractor.extract(text)`. Cascading a signature change
// for an unproven probe is high risk. Default impl falls through to the
// single-text path, so existing implementations and callers are unaffected.
// Only LoCoMo bench + future opt-in pipelines call the new method.

/// Context for a single extraction call. Carries the current turn plus an
/// optional preceding turn for conversational grounding (ISS-178).
///
/// Use [`ExtractionContext::single`] for the standard "just this text" path
/// (preserves pre-ISS-178 behaviour byte-identically). Use
/// [`ExtractionContext::with_prev`] when the previous conversational turn is
/// available — e.g. LoCoMo ingest replaying user/assistant pairs.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ExtractionContext {
    /// Text being extracted now. Required.
    pub current: String,
    /// Immediately preceding turn (any speaker), if available. None = no
    /// prior context, falls through to the single-text extract path.
    pub prev: Option<String>,
}

impl ExtractionContext {
    /// Single-text extraction — semantically identical to calling
    /// `extract(text)` directly. Used by every call site that doesn't have
    ...
    /// Extraction with explicit previous turn. Pass `Some(_)` when the
    /// preceding turn is available; pass `None` (or use [`Self::single`]) to
    /// preserve the legacy single-text path.
    pub fn with_prev(current: impl Into<String>, prev: Option<String>) -> Self {
        Self {
    ...
    /// True iff a previous turn is attached. Implementations override
    /// `extract_with_context` use this to decide whether to enrich the prompt
    /// or fall through to single-turn extraction.
    pub fn has_prev(&self) -> bool {
        self.prev.as_ref().is_some_and(|p| !p.trim().is_empty())
    }
}
...
pub trait MemoryExtractor: Send + Sync {
    ...
    /// Extract with optional preceding-turn context (ISS-178).
    ///
    /// Default implementation falls through to [`Self::extract`] using only
    /// `ctx.current`, preserving pre-ISS-178 behaviour byte-identically for
    /// any implementation that does not override.
    ///
    /// Implementations should override when they can use `ctx.prev` to
    /// disambiguate noun phrases, anchor entities, or otherwise improve
    /// extraction fidelity on the current turn. The override MUST still
    /// return facts grounded in `ctx.current` — `ctx.prev` is for context,
    /// not a source of facts to extract.
    fn extract_with_context(
        &self,
        ctx: &ExtractionContext,
    ) -> Result<Vec<ExtractedFact>, Box<dyn Error + Send + Sync>> {
        // Default: ignore prev, extract from current only.
        self.extract(&ctx.current)
    }
}
...
Hope this helps!"#;
    ...
    // ========================================================================
    // ISS-178: ExtractionContext + default extract_with_context
    // ========================================================================

    /// Test fixture: records what argument the extractor was called with so we
    /// can prove the default impl forwards `ctx.current` and ignores `ctx.prev`.
    struct RecordingExtractor {
        last_call: std::sync::Mutex<Option<String>>,
    }
    ...
    impl MemoryExtractor for RecordingExtractor {
        fn extract(
            ...
            *self.last_call.lock().unwrap() = Some(text.to_string());
            Ok(vec![])
        }
        // No override of extract_with_context — exercises the default impl.
    }

    /// Test fixture: an extractor that DOES override extract_with_context,
    /// proving the override receives the full ExtractionContext (current+prev).
    struct ContextAwareExtractor {
        last_ctx: std::sync::Mutex<Option<ExtractionContext>>,
    }

    impl ContextAwareExtractor {
        fn new() -> Self {
            Self {
        ...
        fn last(&self) -> Option<ExtractionContext> {
            self.last_ctx.lock().unwrap().clone()
        }
    }

    impl MemoryExtractor for ContextAwareExtractor {
        fn extract(
            &self,
            _text: &str,
        ) -> Result<Vec<ExtractedFact>, Box<dyn Error + Send + Sync>> {
            // Should NOT be called when extract_with_context is overridden.
            panic!("ContextAwareExtractor::extract called — override should have intercepted");
        }
    ...
    #[test]
    fn iss178_extraction_context_single_constructor() {
        let ctx = ExtractionContext::single("hello world");
        assert_eq!(ctx.current, "hello world");
        assert!(ctx.prev.is_none());
        assert!(!ctx.has_prev());
    }

    #[test]
    fn iss178_extraction_context_with_prev_some() {
        ...
        assert_eq!(ctx.current, "Researching adoption agencies");
        assert_eq!(
            ctx.prev.as_deref(),
            Some("What were you looking into yesterday?")
        );
        assert!(ctx.has_prev());
    }

    #[test]
    fn iss178_extraction_context_with_prev_none() {
        let ctx = ExtractionContext::with_prev("hello".to_string(), None);
        assert_eq!(ctx.current, "hello");
        assert!(ctx.prev.is_none());
        assert!(!ctx.has_prev());
    }

    #[test]
    fn iss178_has_prev_false_for_whitespace_only() {
        // Whitespace-only prev should be treated as no context — avoids
        // accidentally enriching the prompt with empty turns.
        let ctx = ExtractionContext::with_prev("text".to_string(), Some("   ".to_string()));
        assert!(!ctx.has_prev());
    }

    #[test]
    fn iss178_default_impl_falls_through_to_extract_with_current_only() {
        // Default impl on the trait must call self.extract(&ctx.current),
        // ignoring ctx.prev entirely. Proves byte-identical behaviour for
        // implementations that don't override.
        let ext = RecordingExtractor::new();
        let ctx = ExtractionContext::with_prev(
            "current turn text".to_string(),
            Some("prev turn that should be ignored".to_string()),
        );
        let result = ext.extract_with_context(&ctx);
        assert!(result.is_ok());
        assert_eq!(ext.last(), Some("current turn text".to_string()));
    }

    #[test]
    fn iss178_default_impl_single_ctx_equivalent_to_direct_extract() {
        // ExtractionContext::single(text) + default extract_with_context
        // must be observationally identical to extract(text).
        let ext1 = RecordingExtractor::new();
        let _ = ext1.extract("plain text");

        let ext2 = RecordingExtractor::new();
        let _ = ext2.extract_with_context(&ExtractionContext::single("plain text"));

        assert_eq!(ext1.last(), ext2.last());
    }

    #[test]
    fn iss178_override_receives_full_context_including_prev() {
        // When extract_with_context is overridden, the override must receive
        // the full ExtractionContext (both current and prev). This is the
        // contract any prev-turn-aware implementation depends on.
        let ext = ContextAwareExtractor::new();
        let ctx = ExtractionContext::with_prev(
            "current".to_string(),
