...
pub use accumulator::{
    EmpathyAccumulator, EmpathyTrend, MIN_EVENTS_FOR_SUGGESTION, NEGATIVE_THRESHOLD,
};
/// Backward-compat aliases
pub type EmotionalAccumulator<'a> = EmpathyAccumulator<'a>;
pub type EmotionalTrend = EmpathyTrend;
pub use alignment::{
    calculate_importance_boost, find_aligned_drives, score_alignment, score_alignment_hybrid,
    DriveEmbeddings, ALIGNMENT_BOOST,
};
pub use feedback::{
    ActionStats, BehaviorFeedback, BehaviorLog, LOW_SCORE_THRESHOLD, MIN_ATTEMPTS_FOR_SUGGESTION,
};
pub use mod_io::{Drive, HeartbeatTask, Identity};
pub use subscriptions::{Notification, Subscription, SubscriptionManager};
...
impl EmpathyBus {
    ...
    pub fn get_trends(
        &self,
        conn: &Connection,
    ) -> Result<Vec<EmpathyTrend>, Box<dyn std::error::Error>> {
        let acc = EmpathyAccumulator::new(conn)?;
    ...
    pub fn get_behavior_stats(
        &self,
        conn: &Connection,
    ) -> Result<Vec<ActionStats>, Box<dyn std::error::Error>> {
        let feedback = BehaviorFeedback::new(conn)?;
    ...
    pub fn suggest_soul_updates(
        &self,
        conn: &Connection,
    ) -> Result<Vec<SoulUpdate>, Box<dyn std::error::Error>> {
        let acc = EmpathyAccumulator::new(conn)?;
    ...
    pub fn suggest_heartbeat_updates(
        &self,
        conn: &Connection,
    ) -> Result<Vec<HeartbeatUpdate>, Box<dyn std::error::Error>> {
        let feedback = BehaviorFeedback::new(conn)?;
        let mut suggestions = Vec::new();
...
"#,
    ...
    fn test_process_interaction() {
        ...
        bus.process_interaction(&conn, "test content", 0.8, "coding")
            .unwrap();
        bus.process_interaction(&conn, "test content", 0.6, "coding")
            .unwrap();
    ...
    fn test_suggest_soul_updates() {
        ...
        for _ in 0..15 {
            bus.process_interaction(&conn, "bad experience", -0.8, "debugging")
                .unwrap();
        }
