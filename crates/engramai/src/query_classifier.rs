...
///
/// Public so downstream consumers (e.g. engram-bench answer extraction,
/// ISS-212) can gate date-asking-specific prompt guidance on the same
/// signal the retrieval reservation uses — keeping retrieval and
/// generation aligned on "is this a date-asking query".
pub fn asks_for_date(query: &str) -> bool {
    let lower = query.to_lowercase();
