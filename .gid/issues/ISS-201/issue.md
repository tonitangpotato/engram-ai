...
## lever-(b) entity-set synthesis — conv-26 A/B verdict (2026-06-13)

Same-DB A/B (ingest once, judge twice; flips are real signal, NOT
re-ingest noise). STAMP 20260613T091147Z. Runs ISS201-ESET-{A,B}.
Arm A = entity_sets OFF, Arm B = entity_sets ON.

- overall: A 0.5066 → B 0.5197 (+1.3pp); net flip +2 (11 gain / 9 loss) — WEAK
- by-category net: temporal +5 (7g/2l), multi-hop +1, open-domain -1,
  **single-hop -3 (2g/5l) — REGRESSION**

**Single-hop flips are ALL list/enumeration questions** (gold = multi-item):
- GAIN: #3 "Adoption agencies", #51 "Horse, sunset, sunrise"
- LOSS: #18 "beach, mountains, forest", #19 "dinosaurs, nature",
  #24 "Running, pottery", #43 "abstract art", #52 "Oliver, Luna, Bailey"

**Diagnosis:** entity-set synthesis is a coin-flip on list questions. It
synthesizes multiple memories into one set-memory; sometimes the set is
complete (gains), sometimes it drops/adds items and turns a previously-
correct answer wrong (losses). Net effect on the very list category it
was designed to help is NEGATIVE (5 loss / 2 gain). This confirms the
recall-probe finding (list/aggregate answers are scattered across
memories, not in any single literal) — aggregation IS the right
direction, but the current synthesis quality loses more than it gains.

**Disposition:** ship gate NOT met (LIST/single-hop net negative; ship
rule requires LIST net > 0 AND atomic no-regress). The +1.3pp overall is
within boundary and driven by temporal (off-target, likely a candidate-
pool-distribution side effect, not the intended list mechanism). Does NOT
proceed to conv-44 cross-val (a conv-26-only marginal lift with a
single-hop regression is not worth corpus-general validation per the K=30
