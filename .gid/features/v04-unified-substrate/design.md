...
- [ ] **T37f** Graph-population unification: make the graph store single-file + migrate historical rows. **PREREQUISITE for T37g — verified 2026-05-30, the two-DB split is a POPULATION split, not a field-mapping gap.**
  - **ROOT-FIX REFRAME (verified 2026-05-30): there is no deployed production engram service.** engram is a library consumed by engram-bench. The only callers of `with_pipeline_pool`/`with_graph_store` are the bench harness, examples, and tests — **all pass a separate `*.graph.db` path**, which is why the 816 edges live in a second file. `with_pipeline_pool` (`memory.rs:314`) **already supports same-file mode** (it peeks for a `memories` table → FK ON same-file / OFF separate-file; ISS-046). So the two-file split is a *caller convention*, not an engine constraint. The root fix is therefore two-pronged, NOT a one-shot backfill:
    1. **Default to same-file (stops the bleed at write time).** Change the harness/examples/tests to install the graph store on the **substrate file** (pass the substrate path, or thread the substrate connection into `with_graph_store`). New ingestion then writes the 695 entities / 816 edges directly into the substrate file's `nodes`/`edges`. Confirm same-file mode tolerates the FK-ON path (the graph schema FKs into `memories(id)` — which now resolves in-file).
    2. **Backfill historical separate-file snapshots (covers DBs already split).** A resumable, idempotent migration (`INSERT OR IGNORE` on deterministic id, checkpoint to `backfill_runs` like T19–T25) that reads a pre-existing `*.graph.db`'s `{graph_entities, graph_edges}` and writes them into the substrate file's `nodes`/`edges`. Only needed for DBs created before prong 1 ships.
  - **Why reframe matters**: a pure backfill (prong 2 alone) is a patch — it leaves the engine still *writing* to two files, so the split re-opens on every fresh ingest. Prong 1 is the root fix per SOUL "root fix, not patch". Inspecting the retained conv-26 snapshot showed why prong 2 is still needed: `substrate.db` unified tables held `nodes`=458 memory + **3 entity**, `edges`=221 provenance — semantic graph absent; the real graph (686 entity + 9 topic nodes, **816 `assertion` edges**) lived only in the separate `graph.db`. The Phase C backfill (`substrate/backfill.rs`) never read `graph_entities`/`graph_edges` (`grep -c`=0), and `SqliteGraphStore` (`graph/store.rs:1018`) dual-wrote its own `nodes`/`edges` into `graph.db`, not the substrate file.
  - **Scope (prong 2)**: read `graph.db.{graph_entities, graph_edges}` (or its already-unified `graph.db.{nodes, edges}`) → write substrate-file `nodes`/`edges`. Resumable, idempotent, checkpointed.
  - **edge_kind remap — DECISION REQUIRED (do not code until resolved).** `graph.db` tags all 816 edges `edge_kind='assertion'` with these `predicate`s (verified counts on conv-26 snapshot): `leads_to` 287, `related_to` 274, `uses` 77, `part_of` 61, `is_a` 57, `implements` 22, `caused_by` 21, `depends_on` 17. **`assertion` is not in the GOAL-1.3 enum, AND none of these 8 predicates appear in the §4 canonical predicate table** — the resolution pipeline emits a vocabulary the design never blessed. Two design problems this exposes:
    1. **§4 defines no `causal` edge_kind rows.** GOAL-1.3 lists `causal` as valid, but the §4 table files `causes` under `structural`, and lists zero `causal` predicates. So "remap to causal" has no canonical landing spot — §4 must add `causal` rows first (e.g. `causal: caused_by, leads_to, depends_on`) or everything collapses into `structural`.
    2. **Traversal blindness (§4 line 718): `factual` plan reads `edge_kind IN ('structural','containment')` only.** If T37f remaps the 387 causal-ish edges (`leads_to`+`caused_by`+`depends_on`) to `edge_kind='causal'`, the factual plan will **never traverse them** — they become invisible to retrieval. So the remap target and the plan-traversal predicate set are coupled: changing one without the other silently drops 387/816 edges from multi-hop.
    - **DECISION (locked 2026-05-30, conservative default — potato may override):** map ALL 8 predicates to `edge_kind='structural'`, preserving the open `predicate` value verbatim. Do NOT introduce `causal` in this task. This is the reversible choice: `predicate` still carries `caused_by`/`leads_to`/`depends_on`, so no information is lost (only the coarse index column), and a later structural→causal split is pure-additive (re-tag by predicate + one traversal-set line). The alternative (split causal now) would expand the verifier fingerprint, touch the factual-plan traversal, and add §4 rows — scope creep inside a stability gate whose contract is "consolidate + prove parity", so it's deferred. **Final mapping table:**
      - `is_a`, `part_of`, `same_as`, `related_to`, `uses`, `implements`, `leads_to`, `caused_by`, `depends_on` → `edge_kind='structural'`, `predicate` unchanged.
      - (Carry-over from the §4 canonical table: `mentions`/`derived_from` stay `provenance`, `co_activated` stays `associative` — those are already correct in the substrate file and are NOT part of this graph.db backfill.)
    - **Deferred alternative** (if a future plan needs causal filtering): add `causal` rows to §4, remap `caused_by`/`leads_to`/`depends_on`→`causal`, AND amend the factual-plan traversal (§4 line 718) to `edge_kind IN ('structural','containment','causal')`. Tracked as a follow-up, not v0.4.
    - **This remap must also be applied at the dual-write site (`graph/store.rs:1018`, currently hardcodes `'assertion'`) so new writes stop emitting the off-enum value.** Document the final table here before coding; the verifier I1/I3 fingerprint depends on it.
  - **id mapping**: `graph_entities.id` is BLOB; substrate `nodes.id` is TEXT-UUID. Confirm the BLOB→UUID-string convention used elsewhere (I2 in §8 verifier) and reuse it so traversal joins resolve.
  - **Acceptance**: after T37f, the substrate file's `edges WHERE edge_kind IN (structural,causal)` count ≈ the 816 from graph.db (minus any `assertion` rows deliberately dropped), and `nodes WHERE node_kind='entity'` ≈ 695. Verifier I2/I3 pass against the substrate file (not graph.db). **Blocks T37g.**
- [ ] **T37g** `SqliteGraphStore` read-switch (the retrieval-layer graph traversal). **This is the asymmetry the rest of Phase D missed.** T29.5 switched the *storage-layer* entity readers (`Storage::get_entity`, `find_entities`, `list_entities`, `get_entities_for_memory`) to unified `nodes`/`edges` behind the flag — but the *retrieval-layer* graph traversal used by `AssociativePlan` for multi-hop is a **separate** code path: `SqliteGraphStore` in `graph/store.rs`. It reads exclusively `FROM graph_entities` / `FROM graph_edges` (20+ sites: `edges_of` L1865/L2125/L2185, `memories_mentioning_entity`, `entities_linked_to_memory`, `get_entity_by_id` L1980, etc.) on the **separate `graph.db` file** installed via `with_pipeline_pool`/`with_graph_store`. `grep -rn unified_substrate crates/engramai/src/graph/` returns **0 hits** — the flag has no effect on graph traversal. Without this task, every multi-hop query traverses the legacy `graph.db` tables even at `unified_substrate=true`, and T39's `DROP graph_entities`/`graph_edges` would break retrieval.
  - **Scope**: thread the `unified_substrate` flag into `SqliteGraphStore` construction (it currently has none); add a unified read path for each of the `GraphStore` trait methods that touch `graph_entities`/`graph_edges` (`edges_of`, `memories_mentioning_entity`, `entities_linked_to_memory`, `get_entity_by_id`, and any siblings — enumerate first, same as T29.5 part-0). When the flag is on, read `nodes WHERE node_kind='entity'` + `edges WHERE edge_kind='structural'` (per T37f's locked mapping — all 8 graph predicates land on `structural`) from the **same unified file** instead of the separate `graph.db`. **If T37f's deferred causal-split alternative is ever adopted, this set and §4 line 718's factual-plan traversal must be updated together.**
  - **Field-mapping risk (now handled by T37f)**: graph traversal joins on entity `id` (BLOB on `graph_entities`, TEXT-UUID on `nodes`) and edge weight/predicate. **T37f must have completed the population backfill and edge_kind remap first** — without it the substrate-file unified tables have only 3 entities / 0 semantic edges and the switched reader returns near-empty. After T37f, confirm verifier I2 (byte-equal id mapping) and I3 (≥1 unified edge per legacy semantic edge) pass *against the substrate file* before flipping reads.
  - **Parity proof**: A/B a LoCoMo multi-hop run (conv-26 + conv-44) with graph reads on legacy `graph.db` vs unified, byte-compare the candidate sets per query (reuse the `iss136_candidate_histogram` probe harness). Multi-hop category must stay within ±1pp; candidate-set Jaccard ≥0.95. **Blocks T38/T39.**
  - **Note**: this does NOT fix the 1-hop-only traversal limit (`associative.rs:470` `expanded_entities` collects 1-hop objects only) — that is ISS-070, a separate retrieval-quality task downstream of unification. T37g only switches *which substrate* the existing 1-hop traversal reads from.
- [ ] **T37x** Phase E exit gate: run §5.5.1 AST-strip inventory on `crates/engramai/src/` (with `#[cfg(test)]` masked) and confirm 0 prod hits across all 10 legacy tables **plus `graph_entities`/`graph_edges`** (the audit must cover `crates/engramai/src/graph/` and `crates/engramai/src/retrieval/`, not just `storage.rs` — `SqliteGraphStore` is a prod reader and T37g must have switched it). Full test suite green (lib + integration + all `v04_phase_*`). One-week production soak at `unified_substrate=true` with no quality regression, master LoCoMo within ±1pp of Phase D baseline (0.4013) across ≥2 independent runs. Per §5.5.5.
...
- [ ] **T38** ≥2 weeks of unified-only operation logged at
  ...
  migration helpers and tests. **The reader audit MUST include
  `crates/engramai/src/graph/` and `crates/engramai/src/retrieval/`,
  not only `storage.rs` — `SqliteGraphStore` reads `graph_entities`/
  `graph_edges` and is only switched by T37g. If T37g is incomplete,
  this audit fails (by design).** Acceptance criteria per §5.6.5
  T38 block.
- [ ] **T39** Schema migration `0.4-final`: DROP all 13 legacy tables
  ...
  **and** DROP denormalized columns (`nodes.episode_id`,
  `edges.episode_id` per §7.4). **DEPENDENCY: dropping `graph_edges`/
  `graph_entities` is unsafe until T37g has switched `SqliteGraphStore`
  reads to unified — these tables are read by retrieval-layer graph
  traversal, not just the storage layer. T39 MUST verify T37g landed
  (graph-store AST-strip clean) before issuing those two DROPs.**
