...
- [ ] **T37f** Graph-population backfill: `graph.db` → substrate-file unified tables. **PREREQUISITE for T37g — verified 2026-05-30, the two-DB split is a POPULATION split, not a field-mapping gap.** Inspecting a retained conv-26 snapshot showed: `substrate.db` unified tables hold `nodes`=458 memory + **3 entity**, `edges`=221 provenance (mention edges) — the semantic graph is essentially absent. The real graph lives in the **separate `graph.db` file**: `nodes`=686 entity + 9 topic, `edges`=**816 assertion** (`leads_to`/`related_to`/`uses`/`part_of`/`is_a`/`caused_by`/`depends_on`). The Phase C backfill driver (`substrate/backfill.rs`) **never reads `graph_entities`/`graph_edges`** (`grep -c` = 0) — it only backfilled substrate-local legacy tables. The graph layer (`SqliteGraphStore`, `graph/store.rs:1018`) dual-writes its own `nodes`/`edges` but **into `graph.db`, not the substrate file**. So the 695 entities + 816 semantic edges were never migrated into the substrate-file unified tables, and T37g's "read unified from the same file" is impossible until they physically move.
  ...
  - **edge_kind remap — DECISION REQUIRED (do not code until resolved).** `graph.db` tags all 816 edges `edge_kind='assertion'` with these `predicate`s (verified counts on conv-26 snapshot): `leads_to` 287, `related_to` 274, `uses` 77, `part_of` 61, `is_a` 57, `implements` 22, `caused_by` 21, `depends_on` 17. **`assertion` is not in the GOAL-1.3 enum, AND none of these 8 predicates appear in the §4 canonical predicate table** — the resolution pipeline emits a vocabulary the design never blessed. Two design problems this exposes:
    ...
    - **Proposed mapping (pending potato sign-off):** map ALL 8 predicates to `edge_kind='structural'` (preserving the open `predicate` value verbatim), and do NOT introduce `causal` in this task. Rationale: (a) keeps all 816 edges inside the factual-plan traversal set with zero plan changes, (b) avoids the §4 `causal`-rows gap, (c) `causal` can be split out later as a pure-additive refinement once a plan actually wants to filter on causation. Cost: loses the structural/causal distinction at the edge_kind level, but `predicate` still carries it, so no information is lost — only the coarse index column. **Alternative** (if potato wants causal semantics now): add `causal` rows to §4, remap `caused_by`/`leads_to`/`depends_on`→`causal`, AND amend the factual-plan traversal to `edge_kind IN ('structural','containment','causal')`. More correct, more surface area.
