...
- [ ] **T14** Hebbian (`Storage::record_association` + `record_coactivation_ns` + `record_cross_namespace_coactivation`): dual-write co-activation → `edges(edge_kind='associative')`. All three legacy writers map onto the unified UPSERT in §4.3 with `signal_source` as part of row identity. Cascade refactor: `record_association` `&self` → `&mut self`, propagate through `LinkFormer::storage`, `memory.rs:2585`, `promotion.rs:272` test helper, 5 `former.rs` tests.
