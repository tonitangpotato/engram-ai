...
> **Implementation note (T9, 2026-04-27).** The literal "single SQLite transaction wraps apply + checkpoint advance" wording above describes the *target* atomicity. The T9 implementation in `crates/engramai-migrate/src/processor.rs` (`PipelineRecordProcessor`) achieves equivalent end-to-end semantics via a two-step commit + idempotent replay rather than a single outer transaction, because `GraphStore::apply_graph_delta` opens and commits its own transaction internally. On crash between the two commits, the checkpoint still points at the previous record, so the same record is re-fetched; `apply_graph_delta` then short-circuits to `ApplyReport::already_applied_marker()` (keyed on `(memory_id, delta_hash)`) and the checkpoint advances correctly. Net behavior: at-least-once apply, exactly-once durable effect, monotone checkpoint counters — the "advance-after-commit" property is preserved. **Follow-up:** refactor `apply_graph_delta` to accept a borrowed `&Transaction` so the literal single-tx contract can be restored. Tracked as `task:mig-followup-graph-delta-borrowed-tx` (see also the module-level docstring in `processor.rs` §"Atomicity model").
