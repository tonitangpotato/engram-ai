...
- **v03-graph-layer/design.md**
  ...
  - §4.2 `GraphStore` trait (`search_candidates`, `insert_entity`, `update_entity_cognitive`, `update_entity_embedding`, `touch_entity_last_seen`, `insert_edge`, `find_edges`, `invalidate_edge`, `merge_entities`, `link_memory_to_entities`, `record_extraction_failure`) — the sole write interface for §3.5 plus the `find_edges` read primitive consumed by §3.4.4. Entity create-vs-update is dispatched at the call site (no convenience `upsert_entity`).
