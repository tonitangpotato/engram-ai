...
# Newer issues (post-2026-04-20)

Index below is incomplete — directories under `.gid/issues/` are the source of truth. Summary only:

- **ISS-018** recall-intent-classification — implemented on `wip/dimensional-recall-20260422`
- **ISS-019** dimensional-metadata-write-gap — implemented on `wip/dimensional-recall-20260422`, completed through Step 9 smoke pilot
- **ISS-020** kc-dimensional-awareness — investigation done, dependent on ISS-021
- **ISS-021** subdim-extraction-coverage — fixed (commit `4b02bcc`), storage coverage 74% → 84%, causation coverage 39.5%
- **ISS-022** dimension-vec-string-schema — **open, tech debt** — `Option<String>` → `Vec<String>` refactor for dimension fields; blocks future structured dimension queries
