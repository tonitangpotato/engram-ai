#!/usr/bin/env python3
"""
RUN-0008: LoCoMo conv-26 sessions 1-3 ingest, post ISS-076 dangling-UUID fix.

ISS-076 unified the CreateNew entity UUID source: resolve_edges and
build_new_entity now share EntityResolution.assigned_id, so edges no
longer point at non-existent entity rows.

Phase A only — measures whether removing dangling edges alone moves
retrieval hit@k. ISS-075 (sync embedding) and ISS-077 (in-batch dedup)
are NOT applied here; those are Phase B.

Output: locomo-conv26-iss076.{db,graph.db}
Namespace: locomo-conv26-iss076
"""
...
OUT_DIR = Path("/Users/potato/clawd/projects/engram/.gid/eval-runs/RUN-0008-substrate")
DB_PATH = OUT_DIR / "locomo-conv26-iss076.db"
GRAPH_DB_PATH = OUT_DIR / "locomo-conv26-iss076.graph.db"
