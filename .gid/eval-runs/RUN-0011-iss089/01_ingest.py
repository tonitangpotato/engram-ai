#!/usr/bin/env python3
"""
RUN-0011: LoCoMo conv-26 FULL ingest with --occurred-at (ISS-089 root-fix).

Delta vs RUN-0010: same CLI flags, but underlying engine binary is post-ISS-089.
RUN-0010 silently bypassed the v0.3 pipeline (CLI routed --occurred-at through
add_with_emotion_at → add_raw, skipping extractor + graph emit + resolution
queue). After ISS-089, --occurred-at routes through store_raw Path A — so this
run produces correct created_at values AND a real v0.3 graph substrate.
...
OUT_DIR = Path("/Users/potato/clawd/projects/engram/.gid/eval-runs/RUN-0011-iss089")
