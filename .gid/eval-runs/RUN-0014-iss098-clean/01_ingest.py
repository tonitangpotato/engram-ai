#!/usr/bin/env python3
"""
RUN-0012: LoCoMo conv-26 FULL ingest with --occurred-at (ISS-091 root-fix).

Delta vs RUN-0011: same script + CLI flags, but binary is post-ISS-091
(commit 38c38fe). RUN-0011 reached the v0.3 pipeline (ISS-089 routed
--occurred-at through store_raw Path A) but every job died at finish_run
with `finish_pipeline_run: run not found` — caused by namespace asymmetry
between begin (default ns) and finish (per-job ns set in step 8 §3.5).
ISS-091 stamps namespace BEFORE begin under the same lock window, so
begin/persist/finish all use the same `self.namespace`. This run is the
end-to-end smoke test for that fix; success criterion is `0 finish_run
not found errors` across all 419 turns.
...
OUT_DIR = Path("/Users/potato/clawd/projects/engram/.gid/eval-runs/RUN-0014-iss098-clean")
